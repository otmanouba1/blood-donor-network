...
export const hospitalRequestBlood = async (req, res) => {
  try {
    ...
    if (!labId || !bloodType || !units || units < 1) {
      return res
        .status(400)
        .json({
          success: false,
          message: "labId, bloodType, and units required; units ≥ 1",
        });
    }
    ...
    res
      ...
      .json({
        success: true,
        message: "Blood request sent to lab",
        data: request,
      });
  } catch (err) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error sending blood request" });
  }
};
  ...
  try {
    ...
    const [inventory, requests, hospital] = await Promise.all([
      ...
      Facility.findById(hospitalId).select(
        "history name email phone address lastLogin"
      ),
    ]);
    ...
    res.json({
      success: true,
      stats: { totalUnits, pendingRequests, totalRequests: requests.length },
      inventory,
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load hospital dashboard" });
  }
};
...
export const getHospitalStock = async (req, res) => {
  try {
    ...
    console.error("Get Hospital Stock Error:", err);
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch hospital stock" });
  }
};
...
export const getHospitalHistory = async (req, res) => {
  try {
    const hospitalId = req.user._id;
    const hospital = await Facility.findById(hospitalId).select(
      "history lastLogin"
    );
    if (!hospital)
      return res
    ...
    const history = hospital.history.sort(
      (a, b) => new Date(b.date) - new Date(a.date)
    );
    res.json({ success: true, history });
  } catch (err) {
    console.error("Get Hospital History Error:", err);
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch hospital history" });
  }
};

/* =========================
   BLOOD LAB DONOR MANAGEMENT
   ========================= */
export const getAllDonors = async (req, res) => {
  try {
    ...
    const [donors, total] = await Promise.all([
      Donor.find(filter)
        .select(
          "fullName email phone bloodGroup lastDonationDate donationHistory address"
        )
        .sort(sortOptions)
        .skip(skip)
